...
class EnginePrefixWrapper {
  ...
  parse(response) {
    return this.postprocessSuggestions(this.engine.parse(response));
  }
...
const CompletionSearch = {
  ...
  // Use fetch API instead of XMLHttpRequest for MV3 service worker compatibility
      ...
      .then(response => {
        ...
        if (response.ok) {
          return response.text().then(text => {
            // Return an object similar to XHR for compatibility with existing parse functions
            callback({ status: 200, responseText: text });
          });
        } else {
          callback(null);
        }
    ...
    Utils.setTimeout(this.delay, (handler = (this.mostRecentHandler = () => {
      ...
      if (this.inTransit[completionCacheKey] == null) {
        this.inTransit[completionCacheKey] = new AsyncDataFetcher(callback => {
          ...
          return this.get(searchUrl, url, (response = null) => {
            ...
            let suggestions;
            try {
              suggestions = engine.parse(response)
