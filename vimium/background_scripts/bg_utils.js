...
var BgUtils = {
  ...
  // In MV3 service workers, chrome.extension.getViews() is not available, so we use a message-based approach.
  log: (function() {
    ...
    // In service workers, localStorage is not available
    if (typeof(localStorage) !== 'undefined' && localStorage.autoLaunchLoggingPage) { 
      chrome.windows.create({url: loggingPageUrl, focused: false}); 
    }
    return function(message, sender = null) {
      // In MV3, chrome.extension.getViews is not available in service workers
      ...
      console.log(`[Vimium ${dateString}] ${message}`);
      
      // Also try to log to the logging page via messages if available
      if (typeof(chrome.extension) !== 'undefined' && chrome.extension.getViews) {
        for (let viewWindow of chrome.extension.getViews({type: "tab"})) {
          if (viewWindow.location.pathname === "/vimium/pages/logging.html") {
            // Don't log messages from the logging page itself.
