...
chrome.runtime.onInstalled.addListener(function({ reason }) {
  ...
  // In MV3, use chrome.scripting API instead of chrome.tabs.executeScript
  chrome.tabs.query({ status: "complete" }, function(tabs) {
    for (let tab of tabs) {
      ...
      if (contentScripts.css && contentScripts.css.length > 0) {
        ...
        }).catch(() => {/* Ignore errors for tabs where we can't inject */});
      }
...
// In service workers, use self.crypto instead of window.crypto
const cryptoObj = typeof(window) !== 'undefined' ? window.crypto : (typeof(self) !== 'undefined' ? self.crypto : crypto);
const randomArray = cryptoObj.getRandomValues(new Uint8Array(32)); // 32-byte random token.
...
(function() {
  ...
  fetch(cssUrl)
    .then(response => {
      ...
      throw new Error('Failed to fetch vimium.css');
    })
    .then(cssText => {
      chrome.storage.local.set({vimiumCSSInChromeStorage: cssText});
    })
    .catch(error => {
      console.error('Error loading vimium.css:', error);
    });
})();
...
// In MV3, use chrome.scripting.insertCSS instead of chrome.tabs.insertCSS
chrome.webNavigation.onCommitted.addListener(function({tabId, frameId}) {
  const cssCode = Settings.get("userDefinedLinkHintCss");
  if (cssCode) {
    chrome.scripting.insertCSS({
...
// In MV3 service workers, we cannot use canvas/Image to convert icons.
// Instead, we use path-based icons directly with chrome.action.setIcon
// The iconPaths object maps icon types to their file paths
const iconPaths = {
  [ENABLED_ICON]: { 
    "16": ENABLED_ICON, 
    "19": ENABLED_ICON, 
    ...
    if (request.frameIsFocused) {
      ...
      if (actionApi && actionApi.setIcon) {
        const enabledStateIcon =
          ...
          :
            ENABLED_ICON;
        // Use path-based icons instead of imageData (imageData requires canvas which isn't available in service workers)
        actionApi.setIcon({tabId, path: iconPaths[enabledStateIcon]});
      }
    }
...
// In service workers, window is not available, so we use globalThis
if (typeof(window) !== 'undefined') {
  window.runTests = () => open(chrome.runtime.getURL('tests/dom_tests/dom_tests.html'));
} else {
  globalThis.runTests = () => {
    chrome.tabs.create({ url: chrome.runtime.getURL('tests/dom_tests/dom_tests.html') });
  };
}
