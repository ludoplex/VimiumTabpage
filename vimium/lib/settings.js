...
try { 
  if (typeof(window) != "undefined" && window.chrome == null) { 
...
const Settings = {
  ...
  init() {
    // In service workers, there is no window or localStorage, so we rely entirely on chrome.storage
    const isServiceWorker = typeof(ServiceWorkerGlobalScope) !== 'undefined' && self instanceof ServiceWorkerGlobalScope;
    
    if (!isServiceWorker && typeof(window) != "undefined" && Utils.isExtensionPage() && Utils.isExtensionPage(window.top)) {
      ...
      // In service worker, just run callbacks after storage is loaded
      // The cache will be populated from chrome.storage below
    }
  ...
  nuke(key) {
    // In service workers, there is no localStorage
