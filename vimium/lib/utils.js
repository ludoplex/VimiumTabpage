...
// global scope is `window`. In service workers, the global scope is `self` or `globalThis`.
// The rest of our code attaches exported functions into this global scope.
...
// Note: In service workers, there is no window, so we check for its existence.
if (typeof(window) != "undefined" && window.forTrusted == null) {
...
if (typeof(window) != "undefined" && window.browser && browser.runtime && browser.runtime.getBrowserInfo)
...
var Utils = {
  ...
  // Returns true whenever the current page is the extension's background page or service worker.
  isBackgroundPage() {
    // In MV3, background is a service worker, so check for ServiceWorkerGlobalScope
    if (typeof(ServiceWorkerGlobalScope) !== 'undefined' && self instanceof ServiceWorkerGlobalScope) {
      return true;
    }
    // For MV2 compatibility
    ...
    return typeof(window) != "undefined" && this.isExtensionPage && chrome.extension && chrome.extension.getBackgroundPage &&
